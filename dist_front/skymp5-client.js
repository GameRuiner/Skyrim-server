...
System.register(
	'src/skyrim-platform/skyrimPlatform',
	[],
				...
				exports_2(
					...
					(main = function () {
						...
						});
						{
							...
							skyrimPlatform_1.on('update', function () {
								...
								if (Date.now() - lastBadMenuCheck > 200) {
									...
									noBadMenuOpen =
										badMenus.findIndex(function (menu) {
											return skyrimPlatform_1.Ui.isMenuOpen(menu);
										}) === -1;
								}
						...
						}
						{
							...
							skyrimPlatform_1.on('update', function () {
								var pressed =
									skyrimPlatform_1.Input.isKeyPressed(F6) ||
									(focused_1 && skyrimPlatform_1.Input.isKeyPressed(Escape));
						...
						// var F5 = 0x3F;
						// {
						//     var pressedWas_3 = false;
						//     skyrimPlatform_1.on("update", function () {
						//         var pressed = skyrimPlatform_1.Input.isKeyPressed(F5);
						//         if (pressedWas_3 !== pressed) {
						//             pressedWas_3 = pressed;
						//             if (pressed) {
						//                 skyrimPlatform_1.printConsole("loading url " + url);
						//                 skyrimPlatform_1.browser.loadUrl(url);
						//             }
							...
							switch (_a.label) {
								case 0:
									...
									return [
										...
										f.apply(
											selfCasted,
											snippet.arguments.map(function (arg) {
						...
						return __generator(this, function (_a) {
							switch (_a.label) {
								case 0:
									...
									return [
										4 /*yield*/,
										papyrusClass[snippet.function].apply(
											papyrusClass,
											snippet.arguments.map(function (arg) {
												return deserializeArg(arg);
											})
						...
						return __awaiter(void 0, void 0, void 0, function () {
							...
							return __generator(this, function (_a) {
								if (snippet.class === 'SkympHacks') {
									if (
										snippet.function === 'AddItem' ||
										snippet.function === 'RemoveItem'
									) {
										form = skyrimPlatform_2.Form.from(
											deserializeArg(snippet.arguments[0])
										);
										...
										sp.Sound.from(
											skyrimPlatform_2.Game.getFormEx(soundId)
										).play(skyrimPlatform_2.Game.getPlayer());
										if (count > 0)
											sp.Debug.notification(
												sign + ' ' + form.getName() + ' (' + count + ')'
											);
									} else
								...
								return [
									2 /*return*/,
									snippet.selfId ? runMethod(snippet) : runStatic(snippet),
								];
							});
						});
					})
			...
			execute: function () {
				exports_11(
					...
					(blockConsole = function () {
						if (
							skyrimPlatform_3.settings['skymp5-client']['enable-console'] !==
							true
						) {
							var legalCommands_1 = ['qqq'];
							consoleCommands_1.consoleCommands
								.concat(consoleCommands_1.scriptCommands)
				...
				getCommandExecutor = function (commandName, send, localIdToRemoteId) {
					return function () {
						...
						if (
							...
							!immuneSchema.includes(commandName)
						) {
							skyrimPlatform_3.printConsole(
						...
						send({
							...
							data: { commandName: commandName, args: args },
						});
						if (
							skyrimPlatform_3.storage['_api_onConsoleCommand'] &&
							skyrimPlatform_3.storage['_api_onConsoleCommand']['callback']
						) {
							if (commandName === 'mp') {
								try {
									(_a = skyrimPlatform_3.storage['_api_onConsoleCommand'])[
										'callback'
									].apply(_a, args);
								} catch (e) {
									skyrimPlatform_3.printConsole(
										"'_api_onConsoleCommand' - ",
										e
									);
								}
							}
				...
				exports_11(
					'setUpConsoleCommands',
					(setUpConsoleCommands = function (send, localIdToRemoteId) {
						var command =
							skyrimPlatform_3.findConsoleCommand(' ConfigureUM') ||
							skyrimPlatform_3.findConsoleCommand('test');
						if (command) {
		...
		return {
			...
			execute: function () {
				...
				exports_12(
					...
					(update = function () {
						gAllowGetUp =
							skyrimPlatform_4.Game.getPlayer().getActorValuePercentage(
								'health'
							) >= 0.05;
					})
				);
...
System.register(
	'src/front/components/movementApply',
	['src/skyrim-platform/skyrimPlatform'],
	function (exports_14, context_14) {
		...
		var skyrimPlatform_6,
			...
			applySneaking,
			applyWeapDrawn,
			applyHealthPercentage,
			translateTo,
			teleportIfNeed,
			cellWidth,
			isInDifferentExteriorCell,
			isInDifferentWorldOrCell,
			getPos,
			getDistance;
			...
			execute: function () {
				exports_14(
					...
					(applyMovement = function (refr, m) {
						...
						if (ac) {
							...
							if (m.lookAt) {
								try {
									lookAt = skyrimPlatform_6.Game.findClosestActor(
										m.lookAt[0],
										m.lookAt[1],
										m.lookAt[2],
										128
				...
				applySprinting = function (ac, isSprinting) {
					if (ac.isSprinting() != isSprinting) {
						skyrimPlatform_6.Debug.sendAnimationEvent(
							ac,
							isSprinting ? 'SprintStart' : 'SprintStop'
						);
					}
				};
				applyBlocking = function (ac, m) {
					if (ac.getAnimationVariableBool('IsBlocking') != m.isBlocking) {
						skyrimPlatform_6.Debug.sendAnimationEvent(
							ac,
							m.isBlocking ? 'BlockStart' : 'BlockStop'
						);
						skyrimPlatform_6.Debug.sendAnimationEvent(
							ac,
							m.isSneaking ? 'SneakStart' : 'SneakStop'
						);
					}
				};
				applySneaking = function (ac, isSneaking) {
					var currentIsSneaking =
						ac.isSneaking() || ac.getAnimationVariableBool('IsSneaking');
					if (currentIsSneaking != isSneaking) {
						skyrimPlatform_6.Debug.sendAnimationEvent(
							ac,
							isSneaking ? 'SneakStart' : 'SneakStop'
						);
					}
				};
				exports_14(
					'applyWeapDrawn',
					(applyWeapDrawn = function (ac, isWeapDrawn) {
						if (ac.isWeaponDrawn() !== isWeapDrawn) {
							skyrimPlatform_6.TESModPlatform.setWeaponDrawnMode(
								ac,
								isWeapDrawn ? 1 : 0
							);
						}
				...
				isInDifferentExteriorCell = function (refr, pos) {
					...
					return (
						currentDistanceToPlayer > cellWidth &&
						targetDistanceToPlayer <= cellWidth
					);
				};
				isInDifferentWorldOrCell = function (refr, worldOrCell) {
					return (
						worldOrCell !==
						(refr.getWorldSpace() || refr.getParentCell()).getFormID()
					);
				};
...
System.register(
	'src/front/components/movementGet',
	['src/skyrim-platform/skyrimPlatform'],
			...
			execute: function () {
				exports_15(
					...
					(getMovement = function (refr) {
						...
						return {
							worldOrCell: (
								refr.getWorldSpace() || refr.getParentCell()
							).getFormID(),
							pos: [
								refr.getPositionX(),
								refr.getPositionY(),
				...
				getRunMode = function (ac) {
					...
					if (ac.getFormID() == 0x14) {
						if (
							!skyrimPlatform_7.TESModPlatform.isPlayerRunningEnabled() ||
							speed < 150
						)
							isRunning = false;
					} else {
...
System.register(
	'src/front/components/movement',
	['src/front/components/movementApply', 'src/front/components/movementGet'],
	function (exports_16, context_16) {
		...
		return {
			...
			execute: function () {
				exports_16(
					'applyMovement',
					(applyMovement = movementApply.applyMovement)
				);
...
System.register(
	'src/front/components/animation',
	['src/skyrim-platform/skyrimPlatform', 'src/front/components/movementApply'],
	function (exports_17, context_17) {
		'use strict';
		var skyrimPlatform_8,
			movementApply_2,
			allowedIdles,
			refsWithDefaultAnimsDisabled,
			allowedAnims,
			isIdle,
			applyAnimation,
		...
		return {
			...
			execute: function () {
				...
				exports_17(
					'applyAnimation',
					(applyAnimation = function (refr, anim, state) {
						...
						if (anim.animEventName === 'SkympFakeEquip') {
							...
							if (ac) movementApply_2.applyWeapDrawn(ac, true);
						} else if (anim.animEventName === 'SkympFakeUnequip') {
							var ac = skyrimPlatform_8.Actor.from(refr);
							if (ac) movementApply_2.applyWeapDrawn(ac, false);
							...
							skyrimPlatform_8.Debug.sendAnimationEvent(
								refr,
								anim.animEventName
							);
							if (anim.animEventName === 'GetUpBegin') {
								...
								skyrimPlatform_8.Utility.wait(1).then(function () {
									var ac = skyrimPlatform_8.Actor.from(
										skyrimPlatform_8.Game.getFormEx(refrId_1)
									);
				...
				exports_17(
					'setDefaultAnimsDisabled',
					(setDefaultAnimsDisabled = function (refrId, disabled) {
				...
				AnimationSource = /** @class */ (function () {
					...
					AnimationSource.prototype.getAnimation = function () {
						var _a = this,
							numChanges = _a.numChanges,
							animEventName = _a.animEventName;
						return { numChanges: numChanges, animEventName: animEventName };
					};
					AnimationSource.prototype.onSendAnimationEvent = function (
						animEventName
					) {
						...
						if (
							animEventName.toLowerCase().includes('unequip') &&
							!isTorchEvent
						) {
							this.weapNonDrawnBlocker = Date.now() + 300;
							animEventName = 'SkympFakeUnequip';
						} else if (
							animEventName.toLowerCase().includes('equip') &&
							!isTorchEvent
						) {
							...
							enter: function (ctx) {
								if (refsWithDefaultAnimsDisabled.has(ctx.selfId)) {
									if (ctx.animEventName.toLowerCase().includes('attack')) {
										...
										if (allowedAnims.has(animKey)) {
											...
											skyrimPlatform_8.printConsole(
												'block anim ' + ctx.animEventName
											);
								...
								if (isIdle(ctx.animEventName)) {
									var i = allowedIdles.findIndex(function (pair) {
										return (
											pair[0] === ctx.selfId && pair[1] === ctx.animEventName
										);
									});
									i === -1
										? (ctx.animEventName = '')
...
System.register(
	'src/front/components/look',
	['src/skyrim-platform/skyrimPlatform'],
	function (exports_18, context_18) {
		'use strict';
		var skyrimPlatform_9,
			getLook,
			isVisible,
			applyTints,
			silentVoiceTypeId,
		...
		return {
			...
			execute: function () {
				...
				isVisible = function (argb) {
					return argb > 0x00ffffff || argb < 0;
				};
				exports_18(
					'applyTints',
					(applyTints = function (actor, look) {
						if (!look)
							throw new Error('null look has been passed to applyTints');
						var tints = look.tints.filter(function (t) {
							return isVisible(t.argb);
						});
						var raceWarPaintRegex = /.*Head.+WarPaint.*/;
						var uniWarPaintRegex = /.*HeadWarPaint.*/;
						var raceSpecificWarPaint = tints.filter(function (t) {
							return (
								isVisible(t.argb) && t.texturePath.match(raceWarPaintRegex)
							);
						}).length; // MaleHeadNordWarPaint
						var uniWarPaint = tints.filter(function (t) {
							return isVisible(t.argb) && t.texturePath.match(uniWarPaintRegex);
						...
						if (raceSpecificWarPaint + uniWarPaint > 1) {
							...
							skyrimPlatform_9.printConsole(
								'bad warpaint!',
								raceSpecificWarPaint,
								uniWarPaint
							);
							return;
						}
						...
						tints.forEach(function (tint) {
							skyrimPlatform_9.TESModPlatform.pushTintMask(
								actor,
								tint.type,
								tint.argb,
								tint.texturePath
						...
						var playerBaseId = skyrimPlatform_9.Game.getPlayer()
							.getBaseObject()
							.getFormID();
						if (actor)
							skyrimPlatform_9.TESModPlatform.setFormIdUnsafe(
								actor.getBaseObject(),
								playerBaseId
							);
					})
				...
				applyLookCommon = function (look, npc) {
					var race = skyrimPlatform_9.Race.from(
						skyrimPlatform_9.Game.getFormEx(look.raceId)
					);
					var headparts = look.headpartIds
						.map(function (id) {
							return skyrimPlatform_9.HeadPart.from(
								skyrimPlatform_9.Game.getFormEx(id)
							);
						})
						.filter(function (headpart) {
							return !!headpart;
						});
					skyrimPlatform_9.TESModPlatform.setNpcSex(npc, look.isFemale ? 1 : 0);
					if (race) skyrimPlatform_9.TESModPlatform.setNpcRace(npc, race);
					...
					skyrimPlatform_9.TESModPlatform.resizeHeadpartsArray(
						npc,
						headparts.length
					);
					headparts.forEach(function (v, i) {
						return npc.setNthHeadPart(v, i);
					});
					npc.setFaceTextureSet(
						skyrimPlatform_9.TextureSet.from(
							skyrimPlatform_9.Game.getFormEx(look.headTextureSetId)
						)
					); // setFaceTextureSet supports null argument
					npc.setVoiceType(
						skyrimPlatform_9.VoiceType.from(
							skyrimPlatform_9.Game.getFormEx(silentVoiceTypeId)
						)
					);
					look.options.forEach(function (v, i) {
						return npc.setFaceMorph(v, i);
					});
					look.presets.forEach(function (v, i) {
						return npc.setFacePreset(v, i);
					});
					if (look.name) {
				...
				exports_18(
					'applyLookToPlayer',
					(applyLookToPlayer = function (look) {
						applyLookCommon(
							look,
							skyrimPlatform_9.ActorBase.from(
								skyrimPlatform_9.Game.getPlayer().getBaseObject()
							)
						);
...
System.register(
	'src/front/components/inventory',
	['src/skyrim-platform/skyrimPlatform'],
	function (exports_19, context_19) {
		'use strict';
		var skyrimPlatform_10,
			getRealName,
			cropName,
			checkIfNameIsGeneratedByGame,
			namesEqual,
			extrasEqual,
			hasExtras,
			extractExtraData,
			squash,
			getExtraContainerChangesAsInventory,
			getBaseContainerAsInventory,
			sumInventories,
			getDiff,
			getInventory,
			basesReset,
			...
			execute: function () {
				...
				extrasEqual = function (a, b, ignoreWorn) {
					...
					return (
						...
						((!!a.worn === !!b.worn && !!a.wornLeft === !!b.wornLeft) ||
							ignoreWorn)
					);
				};
				...
				squash = function (inv) {
					...
					inv.entries.forEach(function (e) {
						var same = res.find(function (x) {
							return e.baseId === x.baseId && extrasEqual(x, e);
						});
						if (same) {
							same.count += e.count;
						} else {
					...
					return {
						entries: res.filter(function (x) {
							return x.count !== 0;
						}),
					};
				};
				getExtraContainerChangesAsInventory = function (refr) {
					var extraContainerChanges = skyrimPlatform_10.getExtraContainerChanges(
						refr.getFormID()
					);
				...
				getBaseContainerAsInventory = function (refr) {
					return {
						entries: skyrimPlatform_10.getContainer(
							refr.getBaseObject().getFormID()
						),
					};
				};
				sumInventories = function (lhs, rhs) {
					var leftEntriesWithExtras = lhs.entries.filter(function (e) {
						return hasExtras(e);
					});
					var rightEntriesWithExtras = rhs.entries.filter(function (e) {
						return hasExtras(e);
					});
					var leftEntriesSimple = lhs.entries.filter(function (e) {
						return !hasExtras(e);
					});
					var rightEntriesSimple = rhs.entries.filter(function (e) {
						return !hasExtras(e);
					});
					leftEntriesSimple.forEach(function (e) {
						var matching = rightEntriesSimple.find(function (x) {
							return x.baseId === e.baseId;
						});
						if (matching) {
				...
				exports_19(
					'getDiff',
					(getDiff = function (lhs, rhs, ignoreWorn) {
						var lhsCopy = JSON.parse(JSON.stringify(lhs));
						var rhsCopy = JSON.parse(JSON.stringify(rhs));
						rhsCopy.entries.forEach(function (e) {
							var sameFromLeft = lhsCopy.entries.find(function (x) {
								return x.baseId === e.baseId && extrasEqual(x, e, ignoreWorn);
							});
							if (sameFromLeft) {
						...
						return {
							entries: lhsCopy.entries.filter(function (x) {
								return x.count !== 0;
							}),
						};
					})
				...
				exports_19(
					...
					(applyInventory = function (
						...
						resetBase(refr);
						var diff = getDiff(newInventory, getInventory(refr), ignoreWorn)
							.entries;
						var res = true;
						...
						diff.forEach(function (e, i) {
							...
							if (
								e.count > 1 &&
								skyrimPlatform_10.Ammo.from(
									skyrimPlatform_10.Game.getFormEx(e.baseId)
								)
							) {
								absCount = 1;
							...
							for (var i_1 = 0; i_1 < absCount; ++i_1) {
								if (worn || wornLeft) {
									skyrimPlatform_10.TESModPlatform.pushWornState(
										!!worn,
										!!wornLeft
									);
								...
								if (!f)
									skyrimPlatform_10.printConsole(
										'Bad form ID ' + e.baseId.toString(16)
									);
								else
									skyrimPlatform_10.TESModPlatform.addItemEx(
										refr,
										f,
										oneStepCount,
										e.health ? e.health : 1,
...
System.register(
	'src/front/components/equipment',
	['src/skyrim-platform/skyrimPlatform', 'src/front/components/inventory'],
	function (exports_20, context_20) {
		'use strict';
		var skyrimPlatform_11,
			inventory_1,
			filterWorn,
			removeUnnecessaryExtra,
			getEquipment,
			applyEquipment,
			isBadMenuShown;
		...
		return {
			...
			execute: function () {
				filterWorn = function (inv) {
					return {
						entries: inv.entries.filter(function (x) {
							return x.worn || x.wornLeft;
						}),
					};
				};
				removeUnnecessaryExtra = function (inv) {
					return {
						entries: inv.entries.map(function (x) {
							...
							r.count = skyrimPlatform_11.Ammo.from(
								skyrimPlatform_11.Game.getFormEx(x.baseId)
							)
								? 1000
								: 1;
							delete r.name;
				...
				exports_20(
					'getEquipment',
					(getEquipment = function (ac, numChanges) {
						return {
							inv: inventory_1.getInventory(ac),
				...
				exports_20(
					'applyEquipment',
					(applyEquipment = function (ac, eq) {
						return inventory_1.applyInventory(
							ac,
							removeUnnecessaryExtra(filterWorn(eq.inv)),
							true
						);
					})
				);
				exports_20(
					'isBadMenuShown',
					(isBadMenuShown = function () {
						return (
							skyrimPlatform_11.Ui.isMenuOpen('InventoryMenu') ||
...
System.register(
	...
	function (exports_22, context_22) {
		...
		function processOneActor() {
			...
			var actor = skyrimPlatform_12.Game.findRandomActor(
				pc.getPositionX(),
				pc.getPositionY(),
				pc.getPositionZ(),
				8192
			...
			actor.disable(false).then(function () {
				var ac = skyrimPlatform_12.Actor.from(
					skyrimPlatform_12.Game.getFormEx(actorId)
				);
		...
		return {
			...
			execute: function () {
				...
				skyrimPlatform_13.on('tick', function () {
					...
					gUpdateNeighborFunctions =
						skyrimPlatform_13.storage['updateNeighborFunctions'];
				});
				...
				tryHostIfNeed = function (ac, remoteId) {
					...
					if (!last || Date.now() - last >= 1000) {
						...
						if (
							movementGet_2.getMovement(ac).worldOrCell ===
							movementGet_2.getMovement(skyrimPlatform_13.Game.getPlayer())
								.worldOrCell
						) {
				...
				SpawnProcess = /** @class */ (function () {
					function SpawnProcess(look, pos, refrId, callback) {
						...
						var refr = skyrimPlatform_13.ObjectReference.from(
							skyrimPlatform_13.Game.getFormEx(refrId)
						);
						if (!refr || refr.getFormID() !== refrId) return;
						refr.setPosition.apply(refr, pos).then(function () {
							return _this.enable(look, refrId);
						});
					}
					SpawnProcess.prototype.enable = function (look, refrId) {
						...
						if (look && ac) look_1.applyTints(ac, look);
						refr.enable(false).then(function () {
							return _this.resurrect(look, refrId);
						});
					};
					SpawnProcess.prototype.resurrect = function (look, refrId) {
						...
						return refr
							.setMotionType(skyrimPlatform_13.MotionType.Keyframed, true)
							.then(this.callback);
					};
					return SpawnProcess;
				})();
				...
				FormView = /** @class */ (function () {
					...
					FormView.prototype.update = function (model) {
						...
						if (model.movement) {
							var worldOrCell =
								skyrimPlatform_13.Game.getPlayer().getWorldSpace() ||
						...
						var refId =
							model.refrId && model.refrId < 0xff000000
								? model.refrId
								: undefined;
						if (refId) {
							if (this.refrId !== refId) {
								...
								this.ready = true;
								var refr_1 = skyrimPlatform_13.ObjectReference.from(
									skyrimPlatform_13.Game.getFormEx(this.refrId)
								);
								if (refr_1) {
									var base = refr_1.getBaseObject();
									if (base) dealWithRef(refr_1, base);
								}
							}
						} else {
							var base =
							...
							if (!base) return;
							var refr_2 = skyrimPlatform_13.ObjectReference.from(
								skyrimPlatform_13.Game.getFormEx(this.refrId)
							);
							var respawnRequired =
								!refr_2 ||
							...
							if (respawnRequired) {
								this.destroy();
								refr_2 = skyrimPlatform_13.Game.getPlayer().placeAtMe(
									base,
									1,
									true,
									true
						...
						if (base) {
							...
							if (t >= 38 && t <= 39) {
								...
								if (isHarvested != wasHarvested) {
									...
									if (isHarvested)
										for (var i = 0; i < 20; ++i) {
											ac = skyrimPlatform_13.Game.findRandomActor(
												refr.getPositionX(),
												refr.getPositionY(),
												refr.getPositionZ(),
												10000
									...
									if (isHarvested && ac && ac.getFormID() !== 0x14) {
										...
										refr.disable(false).then(function () {
											var restoredRefr = skyrimPlatform_13.ObjectReference.from(
												skyrimPlatform_13.Game.getFormEx(id_1)
											);
								...
								if (isHarvested != wasHarvested) {
									if (isHarvested) {
										...
										refr.disable(false).then(function () {
											var restoredRefr = skyrimPlatform_13.ObjectReference.from(
												skyrimPlatform_13.Game.getFormEx(id_2)
											);
					...
					FormView.prototype.applyAll = function (refr, model) {
						...
						if (model.look) {
							...
							if (actor && !gPcInJumpState) {
								...
								var screenPoint = skyrimPlatform_13.worldPointToScreenPoint(
									headPos
								)[0];
								var isOnScreen =
									screenPoint[0] > 0 &&
					...
					FormView.prototype.getLookBasedBase = function () {
						var base = skyrimPlatform_13.ActorBase.from(
							skyrimPlatform_13.Game.getFormEx(this.lookBasedBaseId)
						);
						if (!base && this.lookState.look) {
							this.lookBasedBaseId = look_1
								.applyLook(this.lookState.look)
								.getFormID();
						}
				...
				FormViewArray = /** @class */ (function () {
					...
					FormViewArray.prototype.resize = function (newSize) {
						if (this.formViews.length > newSize) {
							this.formViews.slice(newSize).forEach(function (v) {
								return v && v.destroy();
							});
						}
						this.formViews.length = newSize;
					};
					FormViewArray.prototype.updateAll = function (
						model,
						showMe,
						isCloneView
					) {
						...
						for (var i = 0; i < n; ++i) {
							if (
								!forms[i] ||
								(model.playerCharacterFormIdx === i && !showMe)
							) {
								this.destroyForm(i);
								continue;
							}
					...
					FormViewArray.prototype.getRemoteRefrId = function (
						clientsideRefrId
					) {
				...
				WorldView = /** @class */ (function () {
					...
					WorldView.prototype.update = function (model) {
						...
						var showClones =
							skyrimPlatform_13.settings['skymp5-client']['show-clones'];
						var crosshair = skyrimPlatform_13.Game.getCurrentCrosshairRef();
						gCrosshairRefId = crosshair ? crosshair.getFormID() : 0;
						gPcInJumpState = skyrimPlatform_13.Game.getPlayer().getAnimationVariableBool(
							'bInJumpState'
						);
						var pcWorldOrCell =
							skyrimPlatform_13.Game.getPlayer().getWorldSpace() ||
				...
				exports_23(
					'localIdToRemoteId',
					(localIdToRemoteId = function (localFormId) {
				...
				exports_23(
					'remoteIdToLocalId',
					(remoteIdToLocalId = function (remoteFormId) {
...
System.register(
	'src/front/networking',
	['src/skyrim-platform/skyrimPlatform'],
	function (exports_26, context_26) {
		'use strict';
		var skyrimPlatform_14,
			sp,
			handlersMap,
			lastHostname,
			lastPort,
		...
		return {
			...
			execute: function () {
				...
				sp.on('tick', function () {
					skyrimPlatform_14.mpClientPlugin.tick(function (
						packetType,
						jsonContent,
						error
					) {
						...
						handlers.forEach(function (handler) {
							var parse = function () {
								try {
									...
									throw new Error(
										'JSON ' + jsonContent + ' failed to parse: ' + e
									);
								}
							};
				...
				exports_26(
					'on',
					(on = function (packetType, handler) {
				...
				exports_26(
					'send',
					(send = function (msg, reliable) {
						skyrimPlatform_14.mpClientPlugin.send(
							JSON.stringify(msg),
							reliable
						);
					})
				...
				exports_26(
					'reconnect',
					(reconnect = function () {
						return createClientSafe(lastHostname, lastPort);
					})
				);
	...
	function (exports_28, context_28) {
		...
		return {
			...
			execute: function () {
				...
				exports_28(
					'addLoadGameListener',
					(addLoadGameListener = function (onGameLoad) {
						sp.on('loadGame', function () {
							try {
								onGameLoad({
									isCausedBySkyrimPlatform: isCausedBySkyrimPlatform,
								});
							} catch (e) {
				...
				exports_28(
					'loadGame',
					(loadGame = function (pos, rot, worldOrCell, changeFormNpc) {
...
System.register(
	'src/front/updateOwner',
	['src/skyrim-platform/skyrimPlatform', 'src/front/view'],
	function (exports_30, context_30) {
		...
		return {
			...
			execute: function () {
				exports_30(
					'setOwnerModel',
					(setOwnerModel = function (ownerModel) {
		...
		return {
			...
			execute: function () {
					...
					skyrimPlatform_15.once('update', function () {
						try {
							...
							skyrimPlatform_15.printConsole(
								"'eventSources." + ctx._eventName + "' - Added"
							);
						} catch (e) {
							skyrimPlatform_15.printConsole(
								"'eventSources." + ctx._eventName + "' -",
								e
							);
						}
					});
				...
				if (Array.isArray(skyrimPlatform_15.storage['eventSourceContexts'])) {
					skyrimPlatform_15.storage[
						'eventSourceContexts'
					] = skyrimPlatform_15.storage['eventSourceContexts'].filter(function (
						ctx
					) {
						return !ctx._expired;
					});
					skyrimPlatform_15.storage['eventSourceContexts'].forEach(function (
						ctx
					) {
				...
				skyrimPlatform_15.on('tick', function () {
					...
					if (
						verifyStartMoment &&
						Date.now() - verifyStartMoment > maxVerifyDelay
					) {
						maxVerifyDelay *= 2;
						skyrimPlatform_15.printConsole(
				...
				verifySourceCode = function () {
					...
					skyrimPlatform_15.printConsole(
						'Verifying current source code (' + src.length + ' bytes)'
					);
					networking.send(
						{
							t: messages.MsgType.CustomPacket,
					...
					networking.send(
						{
							...
							content: {
								...
								gameData:
									skyrimPlatform_15.settings['skymp5-client']['gameData'],
							},
						},
						true
					);
				...
				RemoteServer = /** @class */ (function () {
								...
								return __generator(this, function (_a) {
									switch (_a.label) {
										...
										case 1:
											...
											skyrimPlatform_15.ObjectReference.from(
												skyrimPlatform_15.Game.getFormEx(msg.target)
											).activate(skyrimPlatform_15.Game.getPlayer(), true);
											(function () {
												return __awaiter(_this, void 0, void 0, function () {
													return __generator(this, function (_a) {
														switch (_a.label) {
															case 0:
																if (
																	!!skyrimPlatform_15.Ui.isMenuOpen(
																		'ContainerMenu'
																	)
																)
																	return [3 /*break*/, 2];
																return [
																	4 /*yield*/,
																	skyrimPlatform_15.Utility.wait(0.1),
																];
															case 1:
																_a.sent();
																return [3 /*break*/, 0];
						...
						skyrimPlatform_15.once('update', function () {
							...
							skyrimPlatform_15.TESModPlatform.moveRefrToPosition(
								...
								skyrimPlatform_15.Cell.from(
									skyrimPlatform_15.Game.getFormEx(msg.worldOrCell)
								),
							...
							skyrimPlatform_15.Utility.wait(0.2).then(function () {
								skyrimPlatform_15.Game.getPlayer().setAngle(
									msg.rot[0],
									msg.rot[1],
									msg.rot[2]
								);
					...
					RemoteServer.prototype.createActor = function (msg) {
						...
						if (msg.props) {
							for (var propName in msg.props) {
								...
								this.worldModel.forms[i_2][propName] = msg.props[propName];
							}
						}
							...
							skyrimPlatform_15.once('update', function () {
								if (!task_1.running) {
									...
									skyrimPlatform_15.TESModPlatform.moveRefrToPosition(
										skyrimPlatform_15.Game.getPlayer(),
										skyrimPlatform_15.Cell.from(
											skyrimPlatform_15.Game.getFormEx(
							...
							skyrimPlatform_15.once('tick', function () {
								skyrimPlatform_15.once('tick', function () {
									if (!task_1.running) {
										task_1.running = true;
										skyrimPlatform_15.printConsole(
											'Using loadGame to spawn player'
										);
										skyrimPlatform_15.printConsole(
											'skinColorFromServer:',
					...
					RemoteServer.prototype.destroyActor = function (msg) {
						...
						if (this.worldModel.playerCharacterFormIdx === i) {
							...
							// TODO: move to a separate module
							skyrimPlatform_15.once('update', function () {
								return skyrimPlatform_15.Game.quitToMainMenu();
							});
						}
						...
						if (msg.open) {
							...
							skyrimPlatform_15.once('update', function () {
								return skyrimPlatform_15.Utility.wait(0.3).then(function () {
									var ironHelment = skyrimPlatform_15.Armor.from(
										skyrimPlatform_15.Game.getFormEx(0x00012e4d)
									);
									skyrimPlatform_15.Game.getPlayer().unequipItem(
										ironHelment,
										false,
										true
									);
							...
							return __awaiter(_this, void 0, void 0, function () {
								...
								return __generator(this, function (_a) {
									spSnippet
										...
										.catch(function (e) {
											return skyrimPlatform_15.printConsole(
												'!!! SpSnippet failed',
												e
											);
										});
					...
					RemoteServer.prototype.updateGamemodeUpdateFunctions = function (
						storageVar,
						functionSources
					) {
						skyrimPlatform_15.storage[storageVar] = JSON.parse(
							JSON.stringify(functionSources)
						);
						for (
							var _i = 0, _a = Object.keys(functionSources);
							_i < _a.length;
							_i++
						) {
							var propName = _a[_i];
							try {
								skyrimPlatform_15.storage[storageVar][propName] = new Function(
									'ctx',
									skyrimPlatform_15.storage[storageVar][propName]
								);
						...
						skyrimPlatform_15.storage[storageVar + '_keys'] = Object.keys(
							skyrimPlatform_15.storage[storageVar]
						);
					};
					RemoteServer.prototype.updateGamemodeData = function (msg) {
						...
						//
						if (
							!Array.isArray(skyrimPlatform_15.storage['eventSourceContexts'])
						) {
					...
					RemoteServer.prototype.getIdManager = function () {
						if (!this.idManager_) this.idManager_ = new idManager_1.IdManager();
			...
			execute: function () {
				handleMessage = function (msgAny, handler_) {
					...
					if (msgType === 'hostStop') {
						...
						var hosted = skyrimPlatform_16.storage['hosted'];
						if (typeof hosted === typeof []) {
							skyrimPlatform_16.storage['hosted'] = hosted.filter(function (x) {
								return x !== target_1;
							});
						}
					}
				...
				for (var i = 0; i < 100; ++i) skyrimPlatform_16.printConsole();
				skyrimPlatform_16.printConsole('Hello Multiplayer');
				skyrimPlatform_16.printConsole(
					'settings:',
					skyrimPlatform_16.settings['skymp5-client']
				);
				...
				SkympClient = /** @class */ (function () {
					function SkympClient() {
						...
						skyrimPlatform_16.on('activate', function (e) {
							lastInv = inventory_4.getInventory(
							...
							if (!target)
								return skyrimPlatform_16.printConsole(
									'localIdToRemoteId returned 0 (target)'
								);
							caster = _this.localIdToRemoteId(caster);
							if (!caster)
								return skyrimPlatform_16.printConsole(
									'localIdToRemoteId returned 0 (caster)'
								);
							_this.sendTarget.send(
								{
									t: messages_3.MsgType.Activate,
									data: { caster: caster, target: target },
								},
								true
							);
							skyrimPlatform_16.printConsole('sendActi', {
								caster: caster,
								target: target,
							});
						});
						...
						skyrimPlatform_16.on('containerChanged', function (e) {
							var oldContainerId = e.oldContainer
								? e.oldContainer.getFormID()
								: 0;
							var newContainerId = e.newContainer
						...
						skyrimPlatform_16.on('containerChanged', function (e) {
							if (e.oldContainer && e.newContainer) {
								if (
									...
									if (!lastInv) lastInv = remoteServer_1.getPcInventory();
									if (lastInv) {
										skyrimPlatform_16.printConsole(2);
										var newInv = inventory_4.getInventory(
											skyrimPlatform_16.Game.getPlayer()
										);
										...
										for (var i = 0; i < diff.entries.length; ++i) {
											skyrimPlatform_16.printConsole(
												'[' + i + '] ' + JSON.stringify(diff.entries[i])
											);
										}
										var msgs = diff.entries.map(function (entry) {
											if (entry.count !== 0) {
												...
												msg['t'] =
													entry.count > 0
														? messages_3.MsgType.PutItem
														: messages_3.MsgType.TakeItem;
										...
										msgs.forEach(function (msg) {
											return _this.sendTarget.send(msg, true);
										});
									}
								}
							}
						...
						skyrimPlatform_16.on('loadGame', function () {
							...
							skyrimPlatform_16.Utility.wait(3).then(function () {
								return (_this.equipmentChanged = true);
							});
						});
					...
					SkympClient.prototype.sendAnimation = function (_refrId) {
						...
						if (
							...
							if (anim.animEventName !== '') {
								...
								this.sendTarget.send(
									{
										t: messages_3.MsgType.UpdateAnimation,
										data: anim,
										_refrId: _refrId,
									},
									false
								...
								if (
									skyrimPlatform_16.storage._api_onAnimationEvent &&
					...
					SkympClient.prototype.sendLook = function (_refrId) {
						...
						if (shown != this.isRaceSexMenuShown) {
							...
							if (!shown) {
								...
								this.sendTarget.send(
									{
										t: messages_3.MsgType.UpdateLook,
										data: look,
										_refrId: _refrId,
									},
									true
					...
					SkympClient.prototype.sendEquipment = function (_refrId) {
						...
						if (this.equipmentChanged) {
							...
							var eq = equipment_3.getEquipment(
								skyrimPlatform_16.Game.getPlayer(),
								this.numEquipmentChanges
							);
							this.sendTarget.send(
								{
									t: messages_3.MsgType.UpdateEquipment,
									data: eq,
									_refrId: _refrId,
								},
								true
					...
					SkympClient.prototype.sendHostAttempts = function () {
						...
						if (!remoteId) return;
						this.sendTarget.send(
							{ t: messages_3.MsgType.Host, remoteId: remoteId },
							false
						);
					};
					SkympClient.prototype.sendInputs = function () {
						var _this = this;
						var hosted =
							typeof skyrimPlatform_16.storage['hosted'] === typeof []
								? skyrimPlatform_16.storage['hosted']
								: [];
	...
	function (exports_33, context_33) {
		...
		return {
			...
			execute: function () {
				...
				realVersion =
					typeof skyrimPlatform_17.getPlatformVersion === 'function'
						? skyrimPlatform_17.getPlatformVersion()
						: 'unknown';
				exports_33(
					'verifyVersion',
		...
		return {
			...
			execute: function () {
				...
				skyrimPlatform_18.on('update', function () {
					...
					var d = new Date();
					var gameHour = skyrimPlatform_18.GlobalVariable.from(
						skyrimPlatform_18.Game.getFormEx(gameHourId)
					);
					gameHour.setValue(
						d.getUTCHours() +
						d.getUTCMinutes() / 60 +
					...
					var gameMonth = skyrimPlatform_18.GlobalVariable.from(
						skyrimPlatform_18.Game.getFormEx(gameMonthId)
					);
					gameMonth.setValue(d.getUTCMonth());
					var gameYear = skyrimPlatform_18.GlobalVariable.from(
						skyrimPlatform_18.Game.getFormEx(gameYearId)
					);
					gameYear.setValue(d.getUTCFullYear() - 2020 + 199);
					var timeScale = skyrimPlatform_18.GlobalVariable.from(
						skyrimPlatform_18.Game.getFormEx(timeScaleId)
					);
				...
				skyrimPlatform_18.on('update', function () {
					...
					var refr = skyrimPlatform_18.ObjectReference.from(
						skyrimPlatform_18.Game.getFormEx(0x42284)
					);
					if (!refr) return;
