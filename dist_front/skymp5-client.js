			...
			exports_2(
				...
				(main = function () {
					...
					}
					{
						...
						skyrimPlatform_1.on('update', function () {
							var pressed =
								skyrimPlatform_1.Input.isKeyPressed(F6) || (focused_1 && skyrimPlatform_1.Input.isKeyPressed(Escape));
...
System.register(
	'src/front/console',
	['src/skyrim-platform/skyrimPlatform', 'src/front/consoleCommands', 'src/front/messages'],
			...
			execute: function () {
				...
				exports_11(
					...
					(setUpConsoleCommands = function (send, localIdToRemoteId) {
						var command =
							skyrimPlatform_3.findConsoleCommand(' ConfigureUM') || skyrimPlatform_3.findConsoleCommand('test');
						if (command) {
		...
		return {
			...
			execute: function () {
				...
				getRunMode = function (ac) {
					...
					var isRunning = true;
					if (ac.getFormID() == 0x14) {
						if (!skyrimPlatform_7.TESModPlatform.isPlayerRunningEnabled() || speed < 150) isRunning = false;
					} else {
		...
		execute: function () {
			exports_18(
				...
				(getLook = function (actor) {
					...
					var numTints =
						skyrimPlatform_9.Game.getPlayer().getFormID() === actor.getFormID()
			...
			exports_18(
				...
				(applyTints = function (actor, look) {
					...
					var raceSpecificWarPaint = tints.filter(function (t) {
						return isVisible(t.argb) && t.texturePath.match(raceWarPaintRegex);
					...
					if (actor) skyrimPlatform_9.TESModPlatform.setFormIdUnsafe(actor.getBaseObject(), playerBaseId);
				})
			);
			...
			applyLookCommon = function (look, npc) {
				...
				var headparts = look.headpartIds
					.map(function (id) {
						return skyrimPlatform_9.HeadPart.from(skyrimPlatform_9.Game.getFormEx(id));
					})
	...
	function (exports_19, context_19) {
		...
		return {
			...
			execute: function () {
				...
				exports_19(
					'applyInventory',
					(applyInventory = function (refr, newInventory, enableCrashProtection, ignoreWorn) {
						if (ignoreWorn === void 0) {
							ignoreWorn = false;
						}
						resetBase(refr);
						...
						diff.forEach(function (e, i) {
							...
							for (var i_1 = 0; i_1 < absCount; ++i_1) {
								...
								if (!f) skyrimPlatform_10.printConsole('Bad form ID ' + e.baseId.toString(16));
								else
									skyrimPlatform_10.TESModPlatform.addItemEx(
										...
										e.soul ? e.soul : 0,
										e.poisonId ? skyrimPlatform_10.Potion.from(skyrimPlatform_10.Game.getFormEx(e.poisonId)) : null,
	...
	function (exports_23, context_23) {
		...
		function dealWithRef(ref, base) {
			...
			if (isFlora) {
				...
				if (hasIngr) ref.setMotionType(skyrimPlatform_13.MotionType.Keyframed, false);
			}
		}
		return {
			setters: [
			...
			execute: function () {
				...
				FormView = /** @class */ (function () {
					...
					FormView.prototype.update = function (model) {
						...
						if (refId) {
							...
							var respawnRequired =
								!refr_2 || !refr_2.getBaseObject() || refr_2.getBaseObject().getFormID() !== base.getFormID();
							if (respawnRequired) {
								this.destroy();
								...
								if (model.look && model.look.name) refr_2.setDisplayName('' + model.look.name, true);
							}
						...
						if (refr) {
							...
							for (
								var _i = 0, gUpdateNeighborFunctionsKeys_1 = gUpdateNeighborFunctionsKeys;
					...
					FormView.prototype.applyAll = function (refr, model) {
						...
						if (model.animation) animation_1.applyAnimation(refr, model.animation, this.animState);
						if (model.look) {
				...
				WorldView = /** @class */ (function () {
					...
					WorldView.prototype.update = function (model) {
						...
						var pcWorldOrCell =
							skyrimPlatform_13.Game.getPlayer().getWorldSpace() || skyrimPlatform_13.Game.getPlayer().getParentCell();
		...
		return {
			...
			execute: function () {
				...
				if (Array.isArray(skyrimPlatform_15.storage['eventSourceContexts'])) {
					skyrimPlatform_15.storage['eventSourceContexts'] = skyrimPlatform_15.storage['eventSourceContexts'].filter(
						function (ctx) {
							return !ctx._expired;
						}
					);
				...
				skyrimPlatform_15.on('update', function () {
					...
					if (Date.now() - pcInvLastApply > 5000) {
						...
						if (pcInv) inventory_3.applyInventory(skyrimPlatform_15.Game.getPlayer(), pcInv, false, true);
					}
				});
								...
								return __generator(this, function (_a) {
									switch (_a.label) {
										...
										case 1:
											...
											skyrimPlatform_15.ObjectReference.from(skyrimPlatform_15.Game.getFormEx(msg.target)).activate(
					...
					RemoteServer.prototype.teleport = function (msg) {
						skyrimPlatform_15.once('update', function () {
							...
							skyrimPlatform_15.TESModPlatform.moveRefrToPosition(
								skyrimPlatform_15.Game.getPlayer(),
								skyrimPlatform_15.Cell.from(skyrimPlatform_15.Game.getFormEx(msg.worldOrCell)),
								skyrimPlatform_15.WorldSpace.from(skyrimPlatform_15.Game.getFormEx(msg.worldOrCell)),
								msg.pos[0],
								msg.pos[1],
								msg.pos[2],
								msg.rot[0],
								...
								msg.equipment
									? {
											entries: msg.equipment.inv.entries.filter(function (x) {
												return !!skyrimPlatform_15.Armor.from(skyrimPlatform_15.Game.getFormEx(x.baseId));
											}),
									  }
									: { entries: [] },
								false
						...
						if (msg.isMe) {
							...
							skyrimPlatform_15.once('update', function () {
								if (!task_1.running) {
									...
									skyrimPlatform_15.TESModPlatform.moveRefrToPosition(
										skyrimPlatform_15.Game.getPlayer(),
										skyrimPlatform_15.Cell.from(skyrimPlatform_15.Game.getFormEx(msg.transform.worldOrCell)),
										skyrimPlatform_15.WorldSpace.from(skyrimPlatform_15.Game.getFormEx(msg.transform.worldOrCell)),
										msg.transform.pos[0],
										msg.transform.pos[1],
										msg.transform.pos[2],
										msg.transform.rot[0],
					...
					RemoteServer.prototype.customPacket = function (msg) {
						switch (msg.content.customPacketType) {
							...
							case 'newClientVersion':
								...
								if (src.length > 0) skyrimPlatform_15.writePlugin('skymp5-client', src);
								break;
						}
					};
							...
							return __awaiter(_this, void 0, void 0, function () {
								...
								return __generator(this, function (_a) {
									spSnippet
										...
										.catch(function (e) {
											return skyrimPlatform_15.printConsole('!!! SpSnippet failed', e);
										});
									return [2 /*return*/];
								});
							});
			...
			execute: function () {
				...
				if (skyrimPlatform_16.storage.targetIp !== targetIp || skyrimPlatform_16.storage.targetPort !== targetPort) {
					...
					skyrimPlatform_16.printConsole(
						'Connecting to ' + skyrimPlatform_16.storage.targetIp + ':' + skyrimPlatform_16.storage.targetPort
					);
				...
				SkympClient = /** @class */ (function () {
					function SkympClient() {
						...
						skyrimPlatform_16.on('activate', function (e) {
							...
							if (!target) return skyrimPlatform_16.printConsole('localIdToRemoteId returned 0 (target)');
							caster = _this.localIdToRemoteId(caster);
							if (!caster) return skyrimPlatform_16.printConsole('localIdToRemoteId returned 0 (caster)');
						...
						skyrimPlatform_16.on('containerChanged', function (e) {
							...
							if (oldContainerId === 0x14 && newContainerId === 0) {
								...
								if (craftInputObjects && craftInputObjects.entries.length) {
									...
									if (!workbench) return skyrimPlatform_16.printConsole('localIdToRemoteId returned 0');
					...
					SkympClient.prototype.sendInputs = function () {
						var _this = this;
						var hosted =
							typeof skyrimPlatform_16.storage['hosted'] === typeof [] ? skyrimPlatform_16.storage['hosted'] : [];
					...
					SkympClient.prototype.resetView = function () {
						...
						skyrimPlatform_16.on('update', function () {
							if (!_this.singlePlayer) view.update(_this.modelSource.getWorldModel());
						});
					};
...
System.register('src/front/version', ['src/skyrim-platform/skyrimPlatform'], function (exports_33, context_33) {
	...
	return {
		...
		execute: function () {
			...
			realVersion =
				typeof skyrimPlatform_17.getPlatformVersion === 'function' ? skyrimPlatform_17.getPlatformVersion() : 'unknown';
			exports_33(
				'verifyVersion',
					...
					if (!requiredVersion.includes(realVersion)) {
						...
						skyrimPlatform_17.Utility.waitMenuMode(0.5).then(function () {
							skyrimPlatform_17.on('update', function () {
								if (!skyrimPlatform_17.Ui.isMenuOpen('MessageBoxMenu')) skyrimPlatform_17.Game.quitToMainMenu();
							});
						});
					}
				})
